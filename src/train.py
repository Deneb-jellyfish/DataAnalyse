...
from utils.dataset import DEFAULT_SPLIT_CSV, load_qm9_dataset, load_split_table, split_from_table
from utils.training import evaluate, seed_everything, train_epoch, write_history_csv


def main() -> None:
    parser = argparse.ArgumentParser(description="Train baseline GNN with random 80/20 QM9 split.")
    ...
    parser.add_argument("--save-path", default="outputs/models/gnn_base.pt")
    parser.add_argument("--log-path", default="outputs/logs/train_gnn_base_metrics.csv")
    ...
    train_loader = DataLoader(split_data["train_set"], batch_size=args.batch_size, shuffle=True)
    test_loader = DataLoader(split_data["test_set"], batch_size=args.batch_size, shuffle=False)

    input_dim = split_data["train_set"][0].x.size(-1)
